          ...
          {listings?.map((item) => (
            ...
            >
              ...
              <div className="pt-2 space-y-4">
                ...
                <p>
                  <span className="font-bold mr-1">{item.buyoutCurrencyValuePerToken.displayValue}</span> {" "}
                  {item.buyoutCurrencyValuePerToken.symbol}
                </p>
                <div className={`flex items-center space-x-1 justify-end text-xs border w-fit ml-auto p-2 rounded-lg text-white ${item.type === ListingType.Direct ? ('bg-blue-500') : ('bg-red-500') }`}>
                  <p>
                    {item.type === ListingType.Direct ? "Buy Now" : "Auction"}
                  </p>
